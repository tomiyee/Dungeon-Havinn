...
export const ItemStackDisplay = (props: PantryCubbyProps) => {
  ...
  const hasBoiledProgress = isDefined(itemStack.item?.boiledProgress) && itemStack.item.boiledProgress !== 0
  const hasChoppedProgress = isDefined(itemStack.item?.choppedProgress) && itemStack.item.choppedProgress !== 0

  return (<Box sx={styles.itemStack} ref={setNodeRef} {...listeners} {...attributes} style={style}>
    <Box flex={1} display='flex' width={"100%"} alignItems="center" justifyContent='center' position={'relative'}>
      <Badge
        badgeContent={quantity > 1 ? quantity : undefined}
        color="primary"
      >
        <img src={icon} alt={itemName} width={itemName !== "" ? 40 : 70} height={itemName !== "" ? 40 : 70} />
      </Badge>
      {hasBoiledProgress && (
        <LinearProgress sx={styles.boiledProgressBar} variant='determinate' value={itemStack.item?.boiledProgress} color='error' />
      )}
      {hasChoppedProgress && (
        <LinearProgress sx={styles.choppedProgressBar} variant='determinate' value={itemStack.item?.choppedProgress} color='warning' />
      )}
    </Box>
    {itemName !== "" &&
