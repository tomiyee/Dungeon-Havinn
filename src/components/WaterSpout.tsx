 ...
 * that can be washed. When an item is placed in the slot and the player clicks the water spout,
...
export const WaterSpout = () => {
  ...
  const handleWaterSpoutClick = () => {
    playSound();
    if (waterSpoutItem && itemStackIsWashable(waterSpoutItem)) {
      // Apply water to the item
      const updatedItemStack = ItemStackUtils.addWater(waterSpoutItem);
      useDungeonHavinnStore.setState({ waterSpoutSlot: updatedItemStack });
    }
  };

  return (
    <Box display="flex" flexDirection="column" gap={2} alignItems="center">
      <Button onClick={handleWaterSpoutClick} sx={styles.image}>
